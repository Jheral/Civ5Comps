...
UnitHandle CvMilitaryAI::FindBestUnitToScrap(bool bLand, bool bDeficitForcedDisband, int& iReturnedScore)
{
	...
	for(pLoopUnit = m_pPlayer->firstUnit(&iUnitLoop); pLoopUnit != NULL; pLoopUnit = m_pPlayer->nextUnit(&iUnitLoop))
	{
		...
		if(!bDeficitForcedDisband)
		{
			...
			*/
			UnitTypes eUpgradeUnit = GetBestUpgradeUnit(pLoopUnit);
...
UnitTypes CvMilitaryAI::GetBestUpgradeUnit(CvUnit* pUnit) const
{
	...
	// Find closest city. 
	// UnitType values are calculated on a city by city bases, so we will decide based on what the closest city think.
	int iDistance;
	int iLoop;
	CvCity* pLoopCity;
	...
	for(pLoopCity = m_pPlayer->firstCity(&iLoop); pLoopCity != NULL; pLoopCity = m_pPlayer->nextCity(&iLoop))
	{
		...
		if(pPlot)
		{
			iDistance = plotDistance(pUnit->getX(), pUnit->getY(), pLoopCity->getX(), pLoopCity->getY());

			if(iDistance < iBestDistance)
			{
				pUpgradeCity = pLoopCity;
				iBestDistance = iDistance;
			}
		}
	}

	// No cities? Why are we even here?
	if (pUpgradeCity == NULL)
		return NO_UNIT;

	for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		eUpgradeUnit = pUnit->HasUpgrade((UnitClassTypes)iI);
		if (eUpgradeUnit != NO_UNIT)
		{
			iValue = 0;
			CvUnitEntry* pkUnitEntry = GC.getUnitInfo(eUpgradeUnit);

			if (pkUnitEntry && pkUnitEntry->GetUnitAIType((int)pUnit->AI_getUnitAIType()))
			{
				iValue = pUpgradeCity->GetCityStrategyAI()->GetUnitProductionAI()->GetWeight(eUpgradeUnit);
			}

			if (iValue > iBestValue)
			{
				eBestUpgradeUnit = eUpgradeUnit;
				iBestValue = iValue;
			}
		}
	}

	return eBestUpgradeUnit;
}
// END Multiple Unit Upgrades
