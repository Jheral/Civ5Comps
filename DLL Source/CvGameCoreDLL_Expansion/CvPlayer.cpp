...
void CvPlayer::doEventChance(CvEventInfo& kEvent, CvCity* pCity, CvUnit* pUnit)
{
	...
	for (int j = 0; j < kEvent.getNumModifiers(); j++)
	{
		...
		bool bPass = false;
		if (pModifier->getModifierType() > EVENTMOD_PLAYER_START && pModifier->getModifierType() < EVENTMOD_PLAYER_END)
		{
		...
		else if (pCity != NULL && pModifier->getModifierType() > EVENTMOD_CITY_START && pModifier->getModifierType() < EVENTMOD_CITY_END)
		{
			if (pCity->checkEventModifier(*pModifier, false))
			{
				bPass = true;
			}
		}
		else if (pUnit != NULL && pModifier->getModifierType() > EVENTMOD_UNIT_START && pModifier->getModifierType() < EVENTMOD_UNIT_END)
		{
			if (pUnit->checkEventModifier(*pModifier, false))
			{
				bPass = true;
			}
		}
		...
		if (bPass && pModifier->getSet() == "default")
		{
...
bool CvPlayer::isEventSetTest(CvEventModifierInfo& kModifier, int iItem, CvCity* pCity, CvUnit* pUnit) const
{
	...
	case EVENTMOD_UNITCLASS:
		iValue = getUnitClassCount((UnitClassTypes)iItem);
		break;

	case EVENTMOD_BUILDINGCLASS:
		iValue = getBuildingClassCount((BuildingClassTypes)iItem);
		break;
