...
int CvPlayer::getYieldModFromEvents(YieldTypes eIndex) const
{
	CvAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	CvAssertMsg(eIndex < NUM_YIELD_TYPES, "eIndex is expected to be within maximum bounds (invalid Index)");
...
//	--------------------------------------------------------------------------------
void CvPlayer::changeYieldModFromEvents(YieldTypes eIndex, int iChange)
{
	CvAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	CvAssertMsg(eIndex < NUM_YIELD_TYPES, "eIndex is expected to be within maximum bounds (invalid Index)");
...
void CvPlayer::doEvents()
{
	...
	bool bValid;
	//We use insert to keep track of scope ID. A scope can have multiple valid enum IDs, so we need a vector inside the vector. 
...
void CvPlayer::doEventChance(CvEventInfo& kEvent, std::map<std::string, std::vector<int> > &asziScopes, CvCity* pCity, CvUnit* pUnit)
{
	...
	{
		if (checkEventModifier(kEvent.getModifier(j), asziScopes, false))
		{
			iChance *= kEvent.getModifier(j).getFactor();
...
bool CvPlayer::checkEventModifier(CvEventModifier& kModifier, std::map<std::string, std::vector<int> > &asziScopes, bool bRequirement)
{
	int iValue = 0;
