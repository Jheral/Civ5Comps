	...
	*/
	...
	if (pProposal->GetEffects()->m_aiYieldPerWonder[YIELD_CULTURE] != 0)
	{
		...
		if (iNumWonders > 0)
		{
			...
			// This was "fixed" in a patch so that AI values culture higher if going for culture victory
			// Which makes no sense when you stop to think about it (see above)
			// However revamped yields is not about changing how the game work, so the patch logic has been applied here
