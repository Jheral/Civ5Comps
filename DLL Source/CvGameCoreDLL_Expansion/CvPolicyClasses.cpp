...
bool CvPlayerPolicies::IsTimeToChooseIdeology() const
{
	...
	*/
	...
	for (int iI = 0; iI < GC.getNumPolicyBranchInfos(); iI++)
	{
		CvPolicyBranchEntry* pkInfo = GC.getPolicyBranchInfo((PolicyBranchTypes)iI);
		if (pkInfo && pkInfo->IsPurchaseByLevel())
		{
			bool bValid = true;
			for (int iJ = 0; iJ < pkInfo->getNumFlagPrereqs(); iJ++)
			{
				if (m_pPlayer->getFlag(pkInfo->getFlagPrereq(iJ)) < pkInfo->getFlagPrereqValue(iJ))
				{
					bValid = false;
				}
			}
			CvPolicyBranchClassEntry* pkClassInfo = GC.getPolicyBranchClassInfo(pkInfo->GetPolicyBranchClass());
			if (pkClassInfo)
			{
				for (int iJ = 0; iJ < pkClassInfo->getNumFlagPrereqs(); iJ++)
				{
					if (m_pPlayer->getFlag(pkClassInfo->getFlagPrereq(iJ)) < pkClassInfo->getFlagPrereqValue(iJ))
					{
						bValid = false;
					}
				}
