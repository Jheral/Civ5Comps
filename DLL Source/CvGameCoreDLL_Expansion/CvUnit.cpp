...
bool CvUnit::CanUpgradeRightNow(bool bOnlyTestVisible) const
{
	...
	{
		if (CanUpgrade(HasUpgrade((UnitClassTypes)iI), bOnlyTestVisible))
			return true;
	}
...
*/
bool CvUnit::CanUpgrade(UnitTypes eUpgradeUnitType, bool bOnlyTestVisible) const
{
	VALIDATE_OBJECT
	...
	if (!m_pUnitInfo->GetUpgradeUnitClass(pUpgradeUnitInfo->GetUnitClassType()))
		return false;
...
UnitTypes CvUnit::HasUpgrade(UnitClassTypes eUpgradeUnitClass) const
{
	UnitTypes eUpgradeUnitType = NO_UNIT;

	if (m_pUnitInfo->GetUpgradeUnitClass((int)eUpgradeUnitClass))
	{
		CvCivilizationInfo& kCiv = GET_PLAYER(getOwner()).getCivilizationInfo();
		eUpgradeUnitType = (UnitTypes) kCiv.getCivilizationUnits((int)eUpgradeUnitClass);
	}
...
*/
CvUnit* CvUnit::DoUpgrade(UnitTypes eUnitType)
{
	VALIDATE_OBJECT
	if (!CanUpgrade(eUnitType, false))
		return this;
// END Multiple Unit Upgrades
