...
function HappinessTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
		...
		local iEventHappiness = pPlayer:GetHappyFromEvents();
		...
		if (iEventHappiness ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText .. "  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_EVENTS", math.abs(iEventHappiness));
		end
		-- END EventEngine
		strText = strText .. "[/COLOR]";
