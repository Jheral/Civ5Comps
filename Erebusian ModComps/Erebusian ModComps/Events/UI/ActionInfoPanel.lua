...
function OnEndTurnClicked()
	...
	if (blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_POLICY
		...
		or blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_EVENT
		-- END EventEngine
...
function OnEndTurnDirty()
	...
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_CHOOSE_IDEOLOGY) then
		...
		iFlashingState = FLASHING_END_TURN;
	-- EventEngine - v0.1, Snarko
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_EVENT) then
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_SUMMARY_EVENT");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_SUMMARY_EVENT_TT");
		buttonDisabled = false;
		iFlashingState = FLASHING_PRODUCTION;
	-- END EventEngine
	else
