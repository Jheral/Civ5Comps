...
g_NameTable[ NotificationTypes.NOTIFICATION_EVENT ] = "Event";
-- END EventEngine
...
function OnNotificationAdded( Id, type, toolTip, strSummary, iGameValue, iExtraGameData, ePlayer )
    ...
    if( name == "Production" or
		...
		-- EventEngine - v0.1, Snarko
		elseif type == NotificationTypes.NOTIFICATION_EVENT then
			local portraitIndex = GameInfo.Events[iGameValue].PortraitIndex;
			print("Step one succesfull!");
			if portraitIndex ~= -1 then
				print(GameInfo.Events[iGameValue].IconAtlas);
				IconHookup( portraitIndex, 80, GameInfo.Events[iGameValue].IconAtlas, instance.EventImage );				
			end
		-- END EventEngine
		end
