...
function UpdateGPT()
	...
	Controls.CityIncomeValue:SetText( Locale.ToNumber( pPlayer:GetYieldFromCitiesTimes100(YieldTypes.YIELD_GOLD) / 100, "#.##" ) );
	-- END Revamped yields
	...
	Controls.TradeIncomeValue:SetText( Locale.ToNumber( pPlayer:GetCityConnectionYieldTimes100(YieldTypes.YIELD_GOLD) / 100, "#.##" ) );

	local pYield = GameInfo.Yields[YieldTypes.YIELD_GOLD];
	local iBaseGold = pYield.CityConnectionBase;
	local iGoldPerPop = pYield.CityConnectionCityPopMultiplier;
	local strTooltip = Locale.ConvertTextKey("TXT_KEY_EO_INCOME_TRADE");
    strTooltip = strTooltip .. "[NEWLINE][NEWLINE]";
	local iTradeRouteGoldModifier = pPlayer:GetCityConnectionTradeRouteYieldModifier();
	-- TODO what about the capital pop multiplier?
	-- END Revamped yields
    ...
    for pCity in pPlayer:Cities() do
        ...
        if( pPlayer:IsCapitalConnectedToCity( pCity ) ) then
			...
			local tradeIncome = pPlayer:GetCityConnectionYieldTimes100( YieldTypes.YIELD_GOLD, pCity ) / 100;
			-- END Revamped yields
