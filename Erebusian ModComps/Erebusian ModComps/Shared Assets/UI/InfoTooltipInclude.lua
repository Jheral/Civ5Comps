...
function GetHelpTextForBuilding(iBuildingID, bExcludeName, bExcludeHeader, bNoMaintenance, pCity)
	...
	-- Culture
	iCulture = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_CULTURE);
	if (pCity ~= nil) then
	...
	-- Faith
	iFaith = Game.GetBuildingYieldChange(iBuildingID, YieldTypes.YIELD_FAITH);
	if (pCity ~= nil) then
...
function GetYieldTooltipHelper(pCity, iYieldType, strIcon)
	...
	-- Revamped yields - v0.1, Snarko
	iBaseYield = iBaseYield + pCity:GetWonderExtraYield(iYieldType);
	iBaseYield = iBaseYield + pCity:GetYieldPerTurnFromTraits(iYieldType);
	if (pCity:GetGarrisonedUnit() ~= nil) then
		iBaseYield = iBaseYield + Players[pCity:GetOwner()]:GetYieldFromGarrison();
	end
	iBaseYield = iBaseYield + pCity:GetYieldPerReligionTimes100() / 100;
	-- END Revamped yields
...
function GetYieldTooltip(pCity, iYieldType, iBase, iTotal, strIconString, strModifiersString)
	...
	-- Revamped yields - v0.1, Snarko
	iYieldFromReligion = iYieldFromReligion + pCity:GetYieldPerReligionTimes100() / 100;
	-- END Revamped yields
	...
	local iYieldFromEvents = pCity:GetBaseYieldRateFromEvents(iYieldType);
	...
	-- Revamped yields - v0.1, Snarko
	local iExtraYieldFromWonders = pCity:GetWonderExtraYield(iYieldType);
	if (iExtraYieldFromWonders ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_WONDERS", iExtraYieldFromWonders, strIconString);
	end
	
	local iYieldFromTraits = pCity:GetYieldPerTurnFromTraits(iYieldType);
	if (iYieldFromTraits ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_TRAITS", iYieldFromTraits, strIconString);
	end
	
	if (pCity:GetGarrisonedUnit() ~= nil) then
		local iYieldFromGarrison = Players[pCity:GetOwner()]:GetYieldFromGarrison();
	end
	-- END Revamped yields
