...
function RefreshYourReligion()
	...
	strText = GetGenericYieldTooltip(YieldTypes.YIELD_FAITH, 0);
	-- Get rid of the first newline, we don't need it here.
	strText = string.gsub(strText, "%[NEWLINE]", "", 1)
	-- This creates slightly larger space between the text strings than before. Ideally, replace [NEWLINE] with something more similar to the old logic.
	local entry = g_FaithModifierManager:GetInstance();
	entry.FaithModifier:SetText(strText);
	-- END Revamped yields
